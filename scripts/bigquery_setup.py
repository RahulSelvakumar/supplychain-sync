...
def main():
    ...
    sql = f"""
    CREATE OR REPLACE TABLE `{GCP_PROJECT}.{BQ_DATASET}.{BQ_TABLE}` AS
    SELECT
    ...
    print(f"✅ Table ready: {GCP_PROJECT}.{BQ_DATASET}.{BQ_TABLE}")

    row = list(client.query(
        f"SELECT COUNT(*) as cnt, COUNTIF(status='AT_RISK') as at_risk "
        f"FROM `{GCP_PROJECT}.{BQ_DATASET}.{BQ_TABLE}`"
    ).result())[0]
    print(f"✅ {row.cnt:,} total rows · {row.at_risk:,} AT_RISK shipments")
